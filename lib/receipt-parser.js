const STOPWORDS = [
  'totale',
  'subtotale',
  'subtot',
  'pagamento',
  'contanti',
  'bancomat',
  'carta',
  'resto',
  'iva',
  'scontrino',
  'numero',
  'p.iva',
  'codice',
  'descrizione',
  'qta',
  'ticket',
  'elettronico',
  'documento commerciale',
  'importo pagato'
];

const PRICE_AT_END_REGEX = /(-?\d{1,3}(?:[\.\s]\d{3})*[,\.\s]\d{1,2}|-?\d+[,\.\s]\d{1,2})\s*(?:€|eur)?\s*$/i;

function parsePrice(raw) {
  if (!raw) return null;

  const normalized = raw
    .replace(/\s+/g, '')
    .replace(/\.(?=\d{3}(\D|$))/g, '')
    .replace(',', '.')
    .replace(/[^0-9.-]/g, '');

  const value = Number.parseFloat(normalized);
  if (!Number.isFinite(value)) return null;
  if (Math.abs(value) > 999.99) return null;
  return Number(value.toFixed(2));
}

function isLikelyTextLine(line) {
  const lettersOnly = line.replace(/[^a-zàèéìòù]/gi, '');
  const compactLength = line.replace(/\s+/g, '').length;
  const letterRatio = compactLength > 0 ? lettersOnly.length / compactLength : 0;
  return lettersOnly.length >= 2 && letterRatio >= 0.45;
}

function hasStopword(line) {
  const low = line.toLowerCase();
  return STOPWORDS.some((stopword) => low.includes(stopword));
}

function isFooterLine(line) {
  const low = line.toLowerCase();
  return [
    'subtot',
    'totale',
    'pagamento',
    'ticket',
    'importo pagato'
  ].some((stopword) => low.includes(stopword));
}

function cleanupItemName(rawName) {
  if (!rawName) return '';
  let cleaned = rawName
    .replace(/^\*+\s*/g, '')
    .replace(/^\d+\s*[xX]\s*/g, '')
    .replace(/[xX]\s*\d+[.,]?\d*\s*$/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim();

  cleaned = cleaned.replace(/\s+(?:\d{1,2}[,.]\d{2}|\d{3,4})$/g, '').trim();
  return cleaned;
}

function parseReceiptText(text) {
  const lines = String(text || '')
    .split(/\r?\n/)
    .map((line) => line.replace(/\s+/g, ' ').trim())
    .filter(Boolean);

  const items = [];
  let pendingName = null;

  for (const line of lines) {
    if (isFooterLine(line) && items.length > 0) {
      break;
    }

    if (hasStopword(line)) {
      pendingName = null;
      continue;
    }

    const priceOnly = line.match(/^(-?\d+(?:[\.,]\d{2})?)\s*(?:€|eur)?$/i);
    if (priceOnly && pendingName) {
      const price = parsePrice(priceOnly[1]);
      const name = cleanupItemName(pendingName);
      if (price !== null && isLikelyTextLine(name) && !hasStopword(name)) {
        items.push({
          id: `${Date.now()}-${items.length}`,
          name,
          price,
          owner: 'both'
        });
      }
      pendingName = null;
      continue;
    }

    const lineWithPrice = line.match(PRICE_AT_END_REGEX);
    if (lineWithPrice) {
      const price = parsePrice(lineWithPrice[1]);
      if (price === null) {
        pendingName = null;
        continue;
      }

      const namePart = line.slice(0, lineWithPrice.index);
      const fallbackName = isLikelyTextLine(namePart) ? namePart : (pendingName || namePart);
      const name = cleanupItemName(fallbackName);

      if (isLikelyTextLine(name) && !hasStopword(name)) {
        items.push({
          id: `${Date.now()}-${items.length}`,
          name,
          price,
          owner: 'both'
        });
      }

      pendingName = null;
      continue;
    }

    if (isLikelyTextLine(line)) {
      pendingName = line;
    }
  }

  return items;
}

module.exports = {
  parsePrice,
  parseReceiptText
};
